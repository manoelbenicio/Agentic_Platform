...
```bash
INIT=$(node "D:/root/.codex/get-shit-done/bin/gsd-tools.cjs" state load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Extract `commit_docs` from init JSON. Resolve debugger model:
```bash
debugger_model=$(node "D:/root/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-debugger --raw)
```
